<div align="center">
  <img src="../assets/images/shared/upc-logo.png" alt="Logo de la Universidad" width="100">
  <br><br>

  <p>
    Universidad Peruana de Ciencias Aplicadas<br>
    Carrera de Ingeniería de Software
  </p>

  <br>

  <p>
    <strong>1ASI0732</strong><br>
    <strong>Diseño de Experimentos de Ingeniería de Software</strong>
  </p>

  <p>
    NRC<br>
    <strong>9097</strong>
  </p>

  <h3><strong>Informe del Trabajo Final</strong></h3>

  <p>
    Docente<br>
    <strong>Julio Manuel Noriega Melendez</strong>
  </p>

  <p>
    Startup<br>
    <strong>TheSuccesfulOnesCorp</strong>
  </p>

  <p>
    Producto<br>
    <strong>SafeSpace</strong>
  </p>

  <p><strong>Integrantes</strong></p>

  <table style="border: none; border-collapse: collapse; margin: 0 auto;">
    <tr style="border: none;">
      <td style="border: none; padding: 3px 15px;"><strong>Código</strong></td>
      <td style="border: none; padding: 3px 15px;"><strong>Apellidos y Nombres</strong></td>
    </tr>
    <tr style="border: none;">
      <td style="border: none; padding: 3px 15px;">u202410093</td>
      <td style="border: none; padding: 3px 15px;">Pajés León, Mauricio Luis</td>
    </tr>
    <tr style="border: none;">
      <td style="border: none; padding: 3px 15px;">U202312566</td>
      <td style="border: none; padding: 3px 15px;">Cayanchi Avila, Milenko Ruben</td>
    </tr>
    <tr style="border: none;">
      <td style="border: none; padding: 3px 15px;">U202313922</td>
      <td style="border: none; padding: 3px 15px;">Ávalos Cordova, Diego Andrés</td>
    </tr>
    <tr style="border: none;">
      <td style="border: none; padding: 3px 15px;">U20241C630</td>
      <td style="border: none; padding: 3px 15px;">Asto Jacome, Jose Gustavo</td>
    </tr>
    <tr style="border: none;">
      <td style="border: none; padding: 3px 15px;">U202412316</td>
      <td style="border: none; padding: 3px 15px;">Diaz Martinez, Alexther Kamil</td>
    </tr>
    <tr style="border: none;">
      <td style="border: none; padding: 3px 15px;">U201919096</td>
      <td style="border: none; padding: 3px 15px;">Arizabal Condori, Jean Niels</td>
    </tr>
  </table>

  <br><br>

  <p>
    <strong>PERÍODO ACADÉMICO</strong><br><br>
    <strong>2026-20</strong>
  </p>
</div>

<div style="page-break-after: always;"></div>
